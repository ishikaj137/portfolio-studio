...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      width = Math.max(global ? window.innerWidth : container.clientWidth, 1);
      height = Math.max(global ? window.innerHeight : container.clientHeight, 1);
    ...
    const updatePointer = event => {
      ...
      if (global) {
        ...
        y = clamp(height - event.clientY, 0, height);
      } else {
  ...
  const rootClassName = [
    'glow-cursor',
    global ? 'glow-cursor--global' : '',
    className
  ].filter(Boolean).join(' ');

  return (
    <div ref={containerRef} className={rootClassName} style={style} {...rest}>
